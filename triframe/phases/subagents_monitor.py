...
sys.path.extend([str(current_dir.parent.parent), str(Path(
    '/home/agent/.agent_code'))])
try:
...
def check_agent_statuses(state: triframeState) ->bool:
    ...
    for agent in current_round:
        ...
        try:
            ...
            agent['status'] = 'failed'
            log_warning(
                f"Error getting status for agent {agent['id']}: {str(e)}")
    return all(agent['status'] in ['concluded', 'failed'] for agent in
        current_round)
...
def create_phase_request(state: triframeState) ->List[StateRequest]:
    ...
    try:
        if not state.active_subagents:
            state.nodes.append(Node(source='tool_output', options=[Option(
                content='No active subagents to monitor', name=
                'launch_subagents', metadata={'error':
                'No active subagents'})]))
            return [StateRequest(state=state, state_model=
                'type_defs.states.triframeState', operations=[log_warning(
                'No active subagents to monitor')], next_phase=
                'triframe/phases/advisor.py')]
        if check_agent_statuses(state):
            tournament = state.get_current_tournament()
            if tournament is None:
                return [StateRequest(state=state, state_model=
                    'type_defs.states.triframeState', operations=[
                    log_system(
        ...
        return [StateRequest(state=state, state_model=
            ...
            'triframe/phases/subagents_monitor.py', delay=10)]
    except Exception as e:
        error_msg = f'Error in monitor phase: {str(e)}'
        state.nodes.append(Node(source='tool_output', options=[Option(
            content=error_msg, name='launch_subagents', metadata={'error':
            str(e)})]))
        return [StateRequest(state=state, state_model=
            'type_defs.states.triframeState', operations=[log_warning(
...
if __name__ == '__main__':
    run_phase('subagents_monitor', create_phase_request,
