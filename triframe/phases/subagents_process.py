...
def process_tournament_results(
    ...
    if not tournament or not state.previous_results or not state.previous_results[-1]:
        operations.append(log_system("No tournament results to process"))
        return operations, state

    # Process all generation results from this round
    ...
    winners = [m.winner_id for m in current_round.matches if m.winner_id]

    # If we have exactly one winner, complete the tournament
    if len(winners) == 1:
        winner_id = winners[0]
        state.complete_tournament(tournament, winner_id)

    # log the diff in state with log_system
    # find the diff, the state does not have a diff method
