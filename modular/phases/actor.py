...
from utils.phase_utils import get_last_completion, get_thinking_block, run_phase
...
def create_phase_request(state: ModularState) -> List[StateRequest]:
    ...
    thinking_block = get_thinking_block(generator_phase_result[0].result.outputs[0])
    message, style = create_function_call_log_message(
        completion, function_call, thinking_block
    )
