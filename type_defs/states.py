...
class Tournament(BaseModel):
    ...
    id: str = Field(
        default_factory=lambda: f"tournament_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    )
...
class triframeState(AgentState):
    ...
    def start_new_tournament(
        ...
        tournament = Tournament(id=tournament_id, initial_agents=agent_ids, task=task)
        self.tournaments.append(tournament)
        return tournament

    def add_tournament_round(
        self, tournament: Tournament, matches: List[TournamentMatch]
    ) -> None:
        """Add a new round to the tournament"""
        round_number = len(tournament.rounds) + 1
        tournament.rounds.append(
            TournamentRound(matches=matches, round_number=round_number)
        )

    def complete_tournament(self, tournament: Tournament, winner_id: str) -> None:
        """Mark a tournament as completed with a winner"""
        tournament.status = "completed"
        tournament.winner_id = winner_id
        self.active_subagents = [
            agent for agent in self.active_subagents if agent["id"] == winner_id
        ]
    ...
    def get_current_tournaments(
        ...
        """Get the current active tournaments or most recent completed tournament if specified"""
        if include_completed:
            return self.tournaments
        return [t for t in self.tournaments if t.status == "in_progress"]
