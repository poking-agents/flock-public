...
class triframeState(AgentState):
    ...
    def start_new_tournament(self, agent_ids: List[str], task: Optional[str
        ]=None, tournament_id: Optional[str]=None) ->Tournament:
        """Start a new tournament with the given agents"""
        if task is None and agent_ids:
            agent = next((a for a in self.active_subagents if a['id'] ==
                agent_ids[0]), None)
        ...
        if tournament_id is None:
            tournament_id = (
                f"tournament_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
        tournament = Tournament(id=tournament_id, initial_agents=agent_ids,
            task=task)
        self.tournaments.append(tournament)
        return tournament
    ...
    def complete_tournament(self, tournament: Tournament, winner_id: str
        ...
        self.active_subagents = [agent for agent in self.active_subagents if
            agent['id'] == winner_id]
    ...
    def get_current_tournament(self, include_completed: bool=False) ->Optional[
        Tournament]:
        """Get the current active tournament or most recent completed tournament if specified"""
        if include_completed:
            return next((t for t in reversed(self.tournaments)), None)
