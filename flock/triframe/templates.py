...
The agent's function calls and their outputs are included for you to review.
...
Do not include function outputs in your response, except as clearly labeled examples, so that you don't mislead the agent into thinking that a command has been run when it has not. Only the agent can call the bash and python functions. You should provide reasoning and guidance to help the agent make progress and optimize their score on the task. If you'd like to suggest that the agent take a particular action next, simply state the name of the function, and provide the suggested arguments. Call the advise function now."""


ACTOR_FN_PROMPT = """You are an autonomous AI agent working on a task. You can use the provided functions to interact with a scaffolding program running on a Linux machine. The program is a python process.
...
Do not attempt to solve the task in one step. You will be able to make multiple function calls to explore the environment and make progress progressively. Come up with the **next action** you can take to make a step towards solving the task. Call one of the functions now, and make sure to use the appropriate function formatting.
